...
public class NaiveAgent_combined implements Runnable {
	...
	public NaiveAgent_combined() {
		...
		collection = database.getCollection("MOMDPAgent");
	...
	public GameState solve_agent()
	{
		...
		//final LogWriter log_combined = new LogWriter("output_baseline.csv");
		//log.appendStartLevel(currentLevel, pigs, birds, blocks, hills, birdOnSling);
		//log_combined.appendStartLevel(currentLevel, pigs, birds, blocks, hills, birdOnSling);
		...
		if (sling != null) {
			...
			if (!pigs.isEmpty()) {
				...
				int dx,dy;
				{
					...
					// System.out.println(pig.getCenter());
						...
						if(scale_diff < 25)
						{
							if(dx < 0)
							{
								...
								try
								{
									...
									//log.flush();
									//log_combined.appendScore(ActionRobot.getCurrentScore(), state);
									//log_combined.appendPigNumber(new_pigs);
									//log.flush(ar.doScreenShot());
									//log_combined.flush();
									Thread.sleep(5000);
								}
	...
	public static void main(String args[]) {
		...
		NaiveAgent_combined na = new NaiveAgent_combined();
		if (args.length > 0)
