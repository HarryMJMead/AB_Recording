...
public class ClientNaiveAgent_new implements Runnable {
	 ...
	 * */
	public ClientNaiveAgent_new(String ip) {
	...
	public ClientNaiveAgent_new(String ip, int id)
	{
		ar = new ClientActionRobotJava(ip);
		tp = new TrajectoryPlanner();
		randomGenerator = new Random();
...

	{
		...
		final LogWriter log = new LogWriter("output_new.csv");
		...
		if (sling != null) {
			...
			if (!pigs.isEmpty()) {
				...
				// Check for clear trajectories
				List<Point> pigsCenter = new ArrayList<>();
				...
				for (ABObject pig : pigs){
					...
					for (Point rPoint : pts){
						if (rPoint != null) {
							...
							int tapTime = tp.getTapTime(sling, rPoint, _tpt, tapInterval);
							Shot poss_shot = new Shot(refPoint.x, refPoint.y, temp_dx, temp_dy, 0, tapTime);
							if (ABUtil.isReachable(vision, _tpt, poss_shot)) {
								pigsCenter.add(_tpt);
				...
				if (!reachableReleasePoint.isEmpty()){
					System.out.println("Found clear Trajectory. ");
					Integer index = randomGenerator.nextInt(reachableReleasePoint.size());
					releasePoint = reachableReleasePoint.get(index);
					Point _tpt = pigsCenter.get(index);
	...
	public static void main(String args[]) {
		...
		ClientNaiveAgent_new na;
		if(args.length > 0)
			na = new ClientNaiveAgent_new(args[0]);
		else
			na = new ClientNaiveAgent_new();
		na.run();
